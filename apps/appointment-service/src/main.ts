...
async function bootstrap() {
  ...
  const port = parseInt(process.env.APPOINTMENT_SERVICE_PORT || '3002', 10);
  ...
  const serviceId = `appointment-service-${port}`;
  consul.agent.service.register({
    id: serviceId,
    name: 'appointment-service',
    address: process.env.CONSUL_HOST ? 'appointment-service' : 'localhost',
    port: port,
    check: {
      name: 'appointment-service-check',
      http: `http://${process.env.CONSUL_HOST ? 'appointment-service' : 'localhost'}:${port}/api/appointments/health`,
      interval: '10s',
      timeout: '5s',
    },
  }).then(() => {
